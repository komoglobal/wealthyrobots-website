#!/usr/bin/env python3
"""
RUN OPTIN SOLVER FIXED
Runs the corrected Folks Finance opt-in solver
"""

from FOLKS_OPTIN_SOLVER_FIXED import FolksOptInSolverFixed
from algosdk.v2client import algod
import os

def main():
    """Run the corrected Folks Finance opt-in solver"""
    print("🚀 RUNNING FOLKS FINANCE OPTIN SOLVER (FIXED)")
    print("=" * 50)
    ...
    try:
        ...
        # Initialize fixed opt-in solver
        print("🔍 Initializing corrected Folks Finance opt-in solver...")
        optin_solver = FolksOptInSolverFixed(algod_client, wallet_address, private_key)
        
        # Run fixed opt-in solving
        print("🚀 Starting corrected opt-in solving...")
        solving_results = optin_solver.run_optin_solving_fixed()
        
        print("\n🎉 CORRECTED OPTIN SOLVING COMPLETE!")
        print("=" * 50)
        ...
        # Summary
        print(f"\n📊 CORRECTED OPTIN SOLVING SUMMARY:")
        print("=" * 40)
        ...
        print(f"❌ Error running corrected opt-in solver: {e}")
        import traceback
        traceback.print_exc()
