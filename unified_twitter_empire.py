...
# Configure logging for the unified empire module
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s [unified_twitter_empire] %(message)s",
    handlers=[
        logging.FileHandler("unified_twitter_empire.log"),
...
class ContentGenerator:
    ...
    def generate_content(self, strategy: Dict) -> Dict:
        ...
        # Fill template
        # Fill template with proper error handling
        ...
        import re
        filled_content = re.sub(r'\{[^}]+\}', '[content]', filled_content)
        
        # Ensure minimum content quality
        if len(filled_content.strip()) < 50:
            filled_content = f"""🧠 AI automation tip: {random.choice(self.content_vars['hooks'])}

Learn how successful entrepreneurs are using AI to automate their businesses.
    ...
    def attempt_post(self) -> Dict:
        ...
        with self.posting_lock:
            ...
            if not can_post:
                ...
                logging.warning(f"Rate limited: {reason}")
            ...
            try:
                ...
                logging.info(f"CEO Strategy: {strategy['type']} ({strategy['period']}) - Affiliate: {strategy['affiliate_chance']*100:.0f}%")
                
                # Generate content
                content_data = self.content_gen.generate_content(strategy)
                logging.info(f"Content: {content_data['length']} chars, viral score: {content_data['viral_score']:.1f}")
                
                # Generate visual if needed
                visual_path = self.visual_coord.generate_visual(content_data)
                if visual_path:
                    logging.info(f"Visual generated: {visual_path}")
                
                # Post safely - simplified version without image support
                ...
                if hasattr(tweet_result, 'get') and tweet_result.get('success'):
                    # Update metrics
                    now = datetime.now()
                    ...
                    logging.info(f"Posted successfully: {tweet_result.get('tweet_id')}")
                    logging.info(f"Posts today: {self.metrics.posts_today}, this month: {self.metrics.posts_this_month}")
                    ...
                    logging.error(f"Post failed: {result['reason']}")
                ...
                logging.exception(f"System error: {e}")
                self._log_result(result)
                return result
...
def main():
    ...
    logging.info("UNIFIED TWITTER EMPIRE STARTING")
    
    # Show current status
    status = empire.get_status()
    logging.info(
        f"Status | can_post={status['can_post']} reason={status['reason']} "
        f"today={status['posts_today']} month={status['posts_this_month']}"
    )
    ...
    if result['success']:
        logging.info(f"Final: success tweet_id={result['tweet_id']}")
    else:
        logging.error(f"Final: failed reason={result['reason']}")
